...
class Handler(watchdog.events.PatternMatchingEventHandler):
    ...
    def on_deleted(self, event):
        print(f"Other document formats for {event.src_path} are being removed.")
        os.remove(event.src_path)


def clear(source_file) -> tuple:
    pBarMax = 2
    widgets = [Percentage(),
               ' ', Bar(),
               ' ', ETA(),
    ...
    docx_file = source_file.replace(".md", ".docx")
    ...
    if os.path.exists(pptx_file):
        pptx_file = source_file.replace(".md", ".pptx")
